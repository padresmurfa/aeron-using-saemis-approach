...
- [x] **Step 6a -- Lock the tier-5 Greater/Lesser structural rule and category list.** Done 2026-05-02. Per user reshape: tier 5 splits into Greater Legendary Beasts (`Vorothar`, the Cataclysmic Beasts — thirteen direct Creator-creations holding the most prominent constellations) and Lesser Legendary Beasts (descended kin; total tier-5 population 70-120). Produced `Aeron/mythopedia/legendary_beasts/README.md` (tier-5 spine canon) and `Aeron/primal_language/structural_principles/vorothar.md` (class headword). Locked the thirteen `Vorothar` categories: Wyrm-Sire, Chimera-Mother, Eldritch-Eye, Colossus-Father, Maw-Eternal, Swarm-Voice, Wraith-Sovereign, Leviathan-Deep, Stormwing, Plague-Mother, Form-Thief, Demon-Sire, Wild-Heart. Updated `mythic_hierarchy.md` tier-5 row, per-tier section, and Canon Boundaries. Updated `constellations.md` with a Prominent Constellations section linking to the `Vorothar`.
  ...
  - Devil's Advocate cleared five constraints (category-collision, tier-3 collision, thirteen-with-hybrid-clause, no-hero-elevation, continent-deferral).
...
- [ ] **Step 6b -- Draft individual `Vorothar` canon entries.** Produce per-beast canon for each of the thirteen `Vorothar` in `Aeron/mythopedia/legendary_beasts/<name>.md`, plus primal-language headword for each in `Aeron/primal_language/structural_principles/`. Recommended batch size: 3-4 beasts per sub-step to keep the linguistic and narrative load tractable. Each `Vorothar` entry needs: primal-language headword (with etymology), the constellation it occupies (named once continents are set), aspects/domains, visual rules derived from primal powers, gaze-resolution scene at peak attunement, awareness/interaction rules, dwelling continent (deferred until continent canon), mana-flavor, dormancy state and stirring triggers, descended-kin lineage definition, and a one-paragraph mythic role. Hybrid lesser-kin lineages may be flagged for later population.
